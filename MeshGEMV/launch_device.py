"""MeshGEMV device launcher (appliance python, cloud artifact_<cfg>)."""
import argparse
import json
import random
import time
...
def main():
    ...
    with SdkRuntime(artifact_id, simulator=args.simulator, disable_version_check=True) as runner:
        ...
        W_u32 = cast_tensor_u32(hc.tile_W(tensor_W, P, Mt, Nt))
    ...
    expected = np.matmul(X.astype(np.float32), tensor_W.astype(np.float32))
